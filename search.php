...
$request=$db->prepare("SELECT text_cat_id,text_status FROM articles INNER JOIN categories ON categories.id=articles.text_cat_id WHERE text_status=:text_status AND text_title LIKE :text_title");
$request->execute([':text_status'=>1,':text_title'=>"%".$search."%"]);
...
<section id="page-content">
	<div class="container">
		<div class="page-title">
			<h1>Axtarışın nəticələri (<?php echo $sum; ?>)</h1>
			<div class="breadcrumb float-left">
				<ul>
					<li><a href="#">Ana Səhifə</a></li>
					<li><a href="#"><?php echo $search;?> başlığına görə axtarış nəticələri(<?php echo $sum; ?>)</a></li>
				</ul>
			</div>
		</div>
		<div id="blog" class="grid-layout post-3-columns m-b-30" data-item="post-item">
			...
			$request=$db->prepare("SELECT * FROM articles INNER JOIN categories ON categories.id=articles.text_cat_id WHERE text_status=:text_status AND text_title LIKE :text_title ORDER BY text_date DESC LIMIT :show,:lim");
			...
			<ul class="pagination">
				...
				if($sum>$lim)
				{
					pagination($s,ceil($sum/$lim),'search.php?search='.$search.'&s=');
				}
				?>
			</ul>
			<?php
		...
		{
			echo '<div class="alert alert-danger">'.$search.' başləğına uyğun nəticə tapılmadı</div>';
		} 
		?>
